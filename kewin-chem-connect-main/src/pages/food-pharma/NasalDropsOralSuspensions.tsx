...
import { ChevronRight, Droplet, Thermometer } from 'lucide-react';

const subSections = [
  { name: 'Oral Suspension', icon: Droplet, color: 'from-blue-500 to-blue-600' },
  { name: 'Nasal Drops', icon: Thermometer, color: 'from-purple-500 to-purple-600' },
];

const NasalDropsOralSuspensions = () => {
  return (
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12 animate-fade-in">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            High-quality nasal drops and oral suspensions for various therapeutic uses.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center max-w-6xl mx-auto">
          {subSections.map((section, index) => (
            <Link
              key={section.name}
              to={`/food-pharma/nasal-oral/${section.name.toLowerCase().replace(/\s+/g, '-')}`}
...
export default NasalDropsOralSuspensions;
