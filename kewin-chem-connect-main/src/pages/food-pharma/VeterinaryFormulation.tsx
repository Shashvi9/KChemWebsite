...
import { ChevronRight, Users, Box, Server, Heart } from 'lucide-react';

const subSections = [
  { name: 'Poultry Products', icon: Users, color: 'from-yellow-400 to-yellow-500' },
  { name: 'Other Products', icon: Box, color: 'from-blue-500 to-blue-600' },
  { name: 'LiveStock Products', icon: Server, color: 'from-green-500 to-green-600' },
  { name: 'Companion Animal Products', icon: Heart, color: 'from-pink-500 to-pink-600' },
];

const VeterinaryFormulation = () => {
  return (
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12 animate-fade-in">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            Wide range of veterinary formulations for poultry, livestock, and companion animals.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center max-w-6xl mx-auto">
          {subSections.map((section, index) => (
            <Link
              key={section.name}
              to={`/food-pharma/veterinary-formulation/${section.name.toLowerCase().replace(/\s+/g, '-')}`}
...
export default VeterinaryFormulation;
